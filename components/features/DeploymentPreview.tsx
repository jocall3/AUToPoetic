 ...
 * generated by other features like the `AiFeatureBuilder`. It fetches the generated files from the local
        ...
        const loadAndRender = async () => {
            ...
            try {
                ...
                if (allFiles.length === 0) {
                    setError('No files generated by AI Feature Builder found in the local database.');
                    setFiles([]);
                    return;
                }
                setFiles(allFiles);
                ...
                for (const file of allFiles) {
                    ...
                    if (file.filePath.endsWith('.css')) mimeType = 'text/css';
                    if (file.filePath.endsWith('.js')) mimeType = 'application/javascript';
                    if (file.filePath.endsWith('.svg')) mimeType = 'image/svg+xml';
                ...
                setError(err instanceof Error ? err.message : 'Failed to load files from the database.');
            } finally {
                setIsLoading(false);
            }
        };
        ...
        <div className="h-full flex flex-col p-4 sm:p-6 lg:p-8 text-text-primary">
            ...
            <div className="flex-grow grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-0">
                <div className="lg:col-span-1 bg-surface p-4 border border-border rounded-lg overflow-y-auto">
                    <h3 className="font-bold mb-2">Generated Files ({files.length})</h3>
                    {isLoading ? (
                        <LoadingSpinner />
                    ) : files.length > 0 ? (
                        <ul className="text-sm space-y-1 font-mono">
                            {files.map(f => <li key={f.filePath} className="truncate p-1 bg-background rounded text-text-secondary">{f.filePath}</li>)}
                        </ul>
                    ) : (
                ...
                <div className="lg:col-span-2 bg-background border-2 border-dashed border-border rounded-lg overflow-hidden">
                    ...
                    {error && <div className="flex justify-center items-center h-full text-red-500 p-4 text-center font-semibold">{error}</div>}
                    {!isLoading && !error && files.length > 0 && (
                     ...
                     {!isLoading && !error && files.length === 0 && (
                        <div className="flex flex-col justify-center items-center h-full text-text-secondary p-4 text-center">
                            <p className="font-semibold">Nothing to Preview</p>
                            <p className="text-sm mt-1">Use the 'AI Feature Builder' to generate files first.</p>
                        </div>
                     )}
                </div>
            </div>
