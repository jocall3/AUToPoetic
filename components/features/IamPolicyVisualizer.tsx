 ...
 * @see {@link core-ui} for the atomic UI components used.
 * @see {@link composite-ui} for the composite UI components used.
...
// Assume a new GraphQL hook for IAM testing. This replaces the direct service call.
// import { useTestIamPermissionsMutation } from '../../graphql/hooks';
import { useGlobalState } from '../../contexts/GlobalStateContext.tsx';

// NOTE: The UI components below are assumed to exist in a proprietary UI framework as per architectural directives.
// They replace raw HTML elements with abstracted, themeable, and accessible components.
import { Button, Card, Grid, Input, Select, Textarea, Title, Text, Spinner, Tag, ListItem } from '../ui/core'; 
import { Header, Container, Modal } from '../ui/composite';
import { GcpIcon, SparklesIcon } from '../ui/icons';
...
/** @type {'idle' | 'running' | 'completed' | 'error'} Status of the simulation process. */
type SimulationStatus = 'idle' | 'running' | 'completed' | 'error';
/** @type {'neutral' | 'pending' | 'success' | 'fail' | 'partial'} Visual status of a resource node. */
...
export const IamPolicyVisualizer: React.FC = () => {
    ...
    const handleRunSimulation = useCallback(async () => {
        if (!state.user) {
            setErrorMessage('You must be signed in to run a simulation.');
            return;
        }
        ...
        if (data && data.testIamPermissions) {
    ...
    }, [resources, permissionList, state.user, testPermissions, simulationError]);
    
    const nodeStatusConfig: Record<NodeStatus, { borderColor: string; tagColor: string; label: string }> = {
        neutral: { borderColor: 'border', tagColor: 'default', label: 'Ready' },
        pending: { borderColor: 'warning', tagColor: 'warning', label: 'Pending' },
        success: { borderColor: 'success', tagColor: 'success', label: 'All Granted' },
        fail: { borderColor: 'danger', tagColor: 'danger', label: 'All Denied' },
        partial: { borderColor: 'attention', tagColor: 'attention', label: 'Partial Access' },
    };
    ...
    return (
        <Container fullHeight className="p-4 sm:p-6 lg:p-8 flex flex-col">
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <ul className="space-y-2 max-h-96 overflow-y-auto">
                            {selectedNode.results?.map(res => (
                                <ListItem key={res.permission} className={`flex items-center justify-between p-2 rounded text-sm ${res.granted ? 'bg-success-container' : 'bg-danger-container'}`}>
            ...
            <div className="flex-grow grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-0 mt-6">
                <Card as="aside" className="lg:col-span-1 flex flex-col gap-4 overflow-y-auto">
                    <Title as="h3" size="xl">Simulation Controls</Title>
                    
                    <div>
                        <Text as="label" size="sm" weight="medium">1. Add Resource</Text>
                ...
                <Card as="main" className="lg:col-span-2 bg-background-alt overflow-auto relative">
                        ...
                        {resources.map(res => {
                            ...
                            return (
                                <Card
                                    ...
                                    className={`p-4 transition-all duration-300 border-4 ${res.results ? 'cursor-pointer hover:scale-105' : ''}`}
