...
/** 
 * @interface ParseJsonWorkerResult
 * @description The result returned by the 'parse-json' worker task.
 */
interface ParseJsonWorkerResult {
    parsedData: any;
    error?: string;
...
/**
 * Transforms a JavaScript object/value into the JsonNode structure for tree rendering.
 * @param {string} key - The key/name for this node.
 * @param {any} value - The JavaScript value to transform.
 * @returns {JsonNode} The transformed node.
 */
function transformToJsonNode(key: string, value: any): JsonNode {
    if (Array.isArray(value)) {
        return {
            key,
            type: 'array',
            value: value.map((item, index) => transformToJsonNode(String(index), item)),
    ...
    if (typeof value === 'object' && value !== null) {
        ...
        return {
            key,
            type: 'object',
            value: entries.map(([k, v]) => transformToJsonNode(k, v)),
 ...
 */
const useJsonParser = (jsonText: string) => {
    ...
    useEffect(() => {
        const handler = setTimeout(async () => {
            ...
            setError(null);
            try {
                const result = await workerPoolManager.submitTask<ParseJsonWorkerResult, { jsonString: string }>(
                    'parse-json',
                    { jsonString: jsonText }
                );
                if (result.error) {
                ...
                const transformedTree = transformToJsonNode('root', result.parsedData);
...
const JsonTreeNodeView: React.FC<{ node: JsonNode, isRoot?: boolean }> = React.memo(({ node, isRoot = false }) => {
    ...
    if (node.type === 'value') {
        ...
        let valueClass = 'text-blue-600 dark:text-blue-400'; // Default for numbers/booleans
        if (valueType === 'string') valueClass = 'text-green-700 dark:text-green-400';
        if (node.value === null) valueClass = 'text-gray-500';
            ...
            {isOpen && (
                <div>
                    {children.map((childNode, index) => (
                        <JsonTreeNodeView key={`${childNode.key}-${index}`} node={childNode} />
                    ))}
...
JsonTreeNodeView.displayName = 'JsonTreeNodeView';
...
export const JsonTreeNavigator: React.FC<{ initialData?: object }> = ({ initialData }) => {
    ...
    const { isParsing, tree, error } = useJsonParser(jsonInput);
        ...
        <div className="h-full flex flex-col p-4 sm:p-6 lg:p-8 text-text-primary">
            <header className="mb-4 flex-shrink-0">
                <h1 className="text-2xl font-bold flex items-center">
                    <div className='w-6 h-6'><FileCodeIcon /></div>
            ...
            <div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-4 min-h-0">
                <div className="flex flex-col h-full bg-surface border border-border rounded-lg p-4">
                ...
                <div className="flex flex-col h-full bg-surface border border-border rounded-lg p-4">
                    <div className="flex justify-between items-center mb-2">
                        ...
                        {isParsing && <div className="text-xs animate-pulse">Parsing...</div>}
                    </div>
                    <div className="flex-grow p-2 bg-background border border-border rounded-md overflow-y-auto font-mono text-sm">
                        {error && <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-md"><div className="font-bold text-red-700">Parsing Error</div><div className="text-sm text-red-600 font-mono mt-1">{error}</div></div>}
                        {tree && !error && <JsonTreeNodeView node={tree} isRoot />}
                        {!tree && !error && !isParsing && <div className="text-text-secondary p-4">Enter valid JSON to view the tree.</div>}
                    </div>
                </div>
            </div>
        </div>
