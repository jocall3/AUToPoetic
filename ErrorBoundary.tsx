...
import { debugErrorStream } from './services/aiService';
import { SparklesIcon } from './components/icons';
import { MarkdownRenderer, LoadingSpinner } from './components/shared/index';
...
export class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  /**
   * @constructor
   * @param {ErrorBoundaryProps} props - The component's properties.
   */
   ...
   * @returns {Partial<ErrorBoundaryState>} An object representing the state update.
   * @static
    ...
    if (this.state.hasError) {
      return (
        <div className="w-full h-full flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8 bg-background text-text-primary">
          <div className="w-full max-w-4xl bg-surface border border-border rounded-lg p-6 shadow-2xl grid grid-cols-1 md:grid-cols-2 gap-6 animate-pop-in">
            <div className="flex flex-col">
              <h1 className="text-2xl font-bold text-red-500 mb-2">An Unexpected Error Occurred</h1>
              <p className="text-text-secondary mb-4">A component has crashed. You can try reloading or ask the AI for debugging help.</p>

              <details className="text-left bg-background p-3 rounded-md text-xs font-mono mb-4 flex-grow overflow-auto border border-border">
              ...
              <div className="flex flex-col sm:flex-row gap-4 mt-auto">
                <button
                  ...
                  className="flex-1 px-4 py-2 bg-amber-400 text-amber-900 font-bold rounded-md hover:bg-amber-300 transition-colors"
                >
                  Reload Application
                </button>
                <button
            ...
            <div className="flex flex-col bg-background rounded-lg p-4 border border-border">
              ...
              <div className="flex-grow overflow-y-auto min-h-[200px]">
                ...
                {!this.state.isAiLoading && !this.state.aiHelp && (
                  <p className="text-text-secondary text-center pt-10">Click "Ask AI" to get debugging suggestions.</p>
                )}
              </div>
            </div>
          </div>
